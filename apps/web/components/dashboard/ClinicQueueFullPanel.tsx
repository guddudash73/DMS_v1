...
function Row({ v, onOpen }: { v: QueueVisit; onOpen: (visitId: string) => void }) {
  ...
  return (
    <button
      ...
      className="flex w-full items-center justify-between gap-3 cursor-pointer rounded-xl border bg-white px-4 py-3 text-left transition hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-black/10"
      title="Open visit"
    >
      ...
      <div className="mx-auto flex h-full w-full max-w-[1200px] flex-col gap-6 2xl:gap-10">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            variant="ghost"
            onClick={onBack}
            className="h-9 rounded-full bg-gray-50 px-4 text-sm font-medium text-gray-800 hover:bg-black hover:text-white cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <Card className="flex h-[520px] flex-col rounded-2xl border-none bg-white p-5 shadow-sm">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-1">
              ...
              {!showLoading &&
                !isError &&
                grouped.queued.map((v) => <Row key={v.visitId} v={v} onOpen={openClinicVisit} />)}
            </div>
          </Card>
          ...
          <Card className="flex h-[520px] flex-col rounded-2xl border-none bg-white p-5 shadow-sm">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-1">
              ...
              {!showLoading &&
                !isError &&
                grouped.inProgress.map((v) => (
                  <Row key={v.visitId} v={v} onOpen={openClinicVisit} />
                ))}
            </div>
          </Card>
          ...
          <Card className="flex h-[520px] flex-col rounded-2xl border-none bg-white p-5 shadow-sm">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-1">
              ...
              {!showLoading &&
                !isError &&
                grouped.done.map((v) => <Row key={v.visitId} v={v} onOpen={openClinicVisit} />)}
            </div>
          </Card>
        </div>
      </div>
